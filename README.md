...
## Настройки системы для разработки 
...
>
>- [Настройка KataGo](#настройка-katago)
>  
...
---
## Краткое введение 
Данное приложение создано студентами СГАУ в качестве практики по предмету "Операционные системы"

Полное название: «GoST (strategical thinking)» – система для игры в Го с поддержкой ИИ и сетевых режимов

«GoST» предназначена для:
- игры в Го на ПК (прежде всего Windows) в режимах человек–человек, человек–ИИ и сетевой PvP;
- анализа и изучения партий, в том числе по файлам SGF;
- хранения и статистического анализа партий и результатов.​
## Основные технологии
Система включает:
- ядро логики Го (C++);
- модуль биндингов (PyBind11);
- клиентское приложение (Python + PySide6);
- модуль интеграции ИИ (GnuGo/KataGo);
- сетевой модуль (WebSockets);
- модуль хранения данных (SQLite).
## Цель проекта
АС «GoST» предназначена для:
- игры в Го на ПК (прежде всего Windows) в режимах человек–человек, человек–ИИ и сетевой PvP;
- анализа и изучения партий, в том числе по файлам SGF;
- хранения и статистического анализа партий и результатов.​

Цели создания системы:
- Обеспечить удобную, современную и полностью офлайндоступную среду для игры в Го с сильным локальным ИИ.
- Объединить в одном продукте игру, анализ, обучение и онлайнигру.
- Создать открытый (opensource) проект, который может развиваться сообществом и расширяться за счёт плагинов/модулей


---
## Быстрый старт
Если Вы обычный пользователь, просто скачайте и распакуйте последнюю версию из **releases**.
...
Для корректного выхода из бота напишите 
```bash 
quit
```
...
---
## Документация
Вся документация к проекту расположена в директории DOCs и включает в себя:
1. AuthAPI – документация к api для аутентификации 
2. BOT_ADAPTERS – документация к файлам адаптерам для KataGo и GnuGo
3. GoEngine – документация к библиотеке python для управления игровым движком на c++
4. Lobby  – документация к лобби режиму
5. LOCAL_NETWORK_GUIDE – инструкция по запуску сервера и тестовых окон лобби режима
6. stucture – дерево проекта с ключевыми директориями и файлами (может отличаться от актуальной версии)
7. requirements – список используемых библиотек python  
8. network_pvp_documentation – полноя документация к многопользовательской игре
