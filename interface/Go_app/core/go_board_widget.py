...
class GoBoardWidget(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.flipped = False        # True только для белых игроков (отражение по вертикали)
    ...
    def mousePressEvent(self, event):
        ...
        if 0 <= screen_row < self.board_size and 0 <= screen_col < self.board_size:
            if self.flipped:
                # Отражение только по вертикали: меняем строку, колонка остаётся
                logical_row = self.board_size - 1 - screen_row
                logical_col = screen_col
            else:
                logical_row = screen_row
                logical_col = screen_col
            logger.debug(f"mousePressEvent: screen=({screen_row},{screen_col}) -> logical=({logical_row},{logical_col})")
    ...
    def draw_last_move_highlight(self, painter):
        ...
        if 0 <= logical_row < self.board_size and 0 <= logical_col < self.board_size:
            if self.flipped:
